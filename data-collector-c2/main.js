...
async function extract_data_from_req(server_response) {
  ...
  let extracted_js = raw_extracted_js
    .replace("data", '"data"')
    .replace(/[ \t\n\r]/g, "")
    .slice(0, -1);
  extracted_js += "}";
  ...
  if (
    typeof extracted_json != "object" ||
    !extracted_json.hasOwnProperty("data") ||
    !Array.isArray(extracted_json.data) ||
    extracted_json.data.length != 24
  ) {
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    result.push({
      ...
      price: parseFloat(extracted_json.data[hour][1]),
    });
  }
